...
& $c -S $r -B $b -G 'Visual Studio 18 2026' -A x64;if($LASTEXITCODE-ne 0){throw 'Configure failed.'}; & $c --build $b --config $Configuration;if($LASTEXITCODE-ne 0){throw 'Build failed.'}; & $c --build $b --config $Configuration --target RUN_TESTS;if($LASTEXITCODE-ne 0){throw 'Tests failed.'}
if($Run){& (Join-Path $b "$Configuration\scope_storage_lifetime.exe");if($LASTEXITCODE-ne 0){throw 'Run failed.'}}

...
$ErrorActionPreference='Stop'; $root=$PSScriptRoot; $build=Join-Path $root 'build'
...
if($Run){& (Join-Path $build "$Configuration\conditionals.exe");if($LASTEXITCODE-ne 0){throw 'Run failed.'}}
